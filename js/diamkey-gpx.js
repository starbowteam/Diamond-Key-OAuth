...
function initGPX() {
    ...
    document.getElementById('saveGpxNameBtn').addEventListener('click', async () => {
        ...
        loadProfilePage(); // обновить карусель
    });
}

// Активация карты при открытии вкладки
const gpxObserver = new MutationObserver(() => {
    if (document.getElementById('page-gpx').classList.contains('active')) {
        initGPX();
        if (gpxMap) gpxMap.invalidateSize();
    }
});
gpxObserver.observe(document.getElementById('page-gpx'), { attributes: true, attributeFilter: ['class'] });
if (document.getElementById('page-gpx').classList.contains('active')) initGPX();
...
function displayGPX(data) {
    ...
    data.tracks.forEach(track => {
        track.segments.forEach(seg => {
            ...
            // Старт и финиш
            if (seg.length) {
                L.marker([seg[0].lat, seg[0].lon], { icon: L.divIcon({ className: '', html: '🚩', iconSize: [32,32], iconAnchor: [16,32] }) }).addTo(gpxLayerGroup);
                L.marker([seg[seg.length-1].lat, seg[seg.length-1].lon], { icon: L.divIcon({ className: '', html: '🏁', iconSize: [32,32], iconAnchor: [16,32] }) }).addTo(gpxLayerGroup);
            }
        });
    });
