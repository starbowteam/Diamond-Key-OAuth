function setupDiamondBot() {
    ...
    async function sendBotMessage() {
        ...
        var { data, error } = await _supabase.from('service_config').select('mistral_api_key').eq('id', 1).maybeSingle();
        ...
        try {
            ...
            var reply = json.choices?.[0]?.message?.content || 'Нет ответа.';
            thinkingEl.remove();
